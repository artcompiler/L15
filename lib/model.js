  ...
  var parse = function parse(src, env) {
    ...
    function numberNode(n0, doScale, roundOnly) {
      ...
      for (i = 0; i < n1.length; i++) {
        if ((ch = n1.charAt(i)) === ",") {
          ...
          } else if (numberFormat === "decimal" && ch !== "0") {
    ...
    function multiplicativeExpr() {
      ...
      while((t = hd()) && !isAdditive(t) && !isRelational(t) &&
        ...
        } else if (args.length > 0 && explicitOperator && isScientific([args[args.length-1], expr])) {
          // 1.2 \times 10 ^ {-3}
          t = args.pop();
          if (isNeg(t)) {
            expr = binaryNode(Model.MUL, [nodeMinusOne, expr]);
    ...
    function isScientific(args) {
      if (args.length === 1) {
        // 1.2, 10^2
        if (args[0].op === Model.NUM &&
            (args[0].args[0].length === 1 || args[0].args[0].indexOf(".") === 1)) {
          return true;
        ...
        var e = args[1];
        if (a.op === Model.NUM &&
            (a.args[0].length === 1 || a.args[0].indexOf(".") === 1) &&
            e.op === Model.POW &&
